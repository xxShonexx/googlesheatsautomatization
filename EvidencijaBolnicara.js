function EvidencijaBolnicari() {
  ...
  const nameRange = namesSheet.getRange('N4:N23');
  ...
  users.forEach(user => {
    if (user) {  // Проверити да ли је име присутно
      ...
      upisiNaslov(rezultatSheet, naslov, currentRow + 1);
      upisiImenaSheetova(rezultatSheet, sheetNames, currentRow);
      ...
      sheetNames.forEach((sheetName, colIndex) => {
        ...
        const { counts, insCount } = izracunajRezultate(sheet, user, targetColor);
        upisiRezultate(rezultatSheet, counts, insCount, currentRow + 1, colIndex + 2);
      });
...
// Функција за уписивање наслова
function upisiNaslov(sheet, naslov, startRow) {
  naslov.forEach((name, index) => {
    sheet.getRange(startRow + index, 1).setValue(name);
  });
}

// Функција за уписивање имена листова
function upisiImenaSheetova(sheet, sheetNames, row) {
  sheetNames.forEach((name, index) => {
    sheet.getRange(row, 2 + index).setValue(name);
  });
}

// Функција за рачунање резултата
function izracunajRezultate(sheet, user, targetColor) {
  const dataRange = sheet.getDataRange();
  const data = dataRange.getValues();
  const formats = dataRange.getFontColors();
  const backgrounds = dataRange.getBackgrounds();
  const counts = { 'БОЛ': 0, 'ДОК': 0 };
  let insCount = "";
...
// Функција за уписивање резултата
function upisiRezultate(sheet, counts, insCount, startRow, col) {
  if (startRow && col) { 
